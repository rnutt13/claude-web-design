...
## Project location
This project lives at `~/Claude Web Design/`. All HTML files should be created inside brand subfolders, e.g. `~/Claude Web Design/nvidia/blackwell-overview.html`.

## Asset locations
All assets are relative to the HTML file's directory (one level deep inside the project).
...
## File naming
Name files descriptively based on purpose, not template type:
`nvidia/blackwell-overview.html`, `salesforce/q2-pipeline-review.html`, `disney/streaming-strategy.html`
